...
import {
  parseTripRequirementBody,
  type TripPlanGenerator,
  type TripPlanSuggestion,
} from '../services/trip-plan-generator';

const INVALID_REQUEST_MESSAGE = '行程规划请求无效，请调整后重试。';
const UNSUPPORTED_TYPE_MESSAGE = '请使用 JSON 提交行程规划。';
const METHOD_MESSAGE = '仅支持生成行程草案。';
...
export function createAiTripPlanRouter(generator?: TripPlanGenerator): Router {
  const router = Router();

  router.all('/ai/trips/plan', async (request: Request, response: Response) => {
    ...
    try {
      const plan: TripPlanSuggestion = await generator.generate(requirement);
      response.status(200).json({ data: { plan } });
    } catch (error) {
